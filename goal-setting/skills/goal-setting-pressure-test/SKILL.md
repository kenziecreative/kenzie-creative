...
2. Otherwise read `goals/vision.md`, `goals/active.md`, and `goals/scorecard.md`. Decide scope:
...
evidence, and never judges whether a goal is the *right* business call.
...
Append the findings to `goals/journal.md` under a `pressure-test` entry tagged with the date, so
they're visible later. Each as a short line naming what it bears on. Do **not** edit
`goals/active.md` or `goals/vision.md` — acting on a finding is the user's call, made by
re-running the relevant Setup stage:
