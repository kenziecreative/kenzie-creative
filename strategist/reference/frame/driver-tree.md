---
name: strategist:reference/frame/driver-tree
...
This is the Frame-stage expression of MECE thinking (Mutually Exclusive, Collectively Exhaustive): each branch should cover a distinct part of the problem, and the branches together should cover all of it.
...
Use a Driver Tree immediately after you have a well-formed problem statement — it is the primary Frame-stage tool for structured decomposition. Reach for it when the problem is complex enough that different teams or workstreams need to investigate different branches in parallel, or when you genuinely don't know yet which dimension of the problem is most important.
...
The insight-stage **Driver Tree** entry covers the same visual format used as a communication artifact — showing how a metric or outcome is driven by its inputs. The Frame-stage version here is a problem-decomposition tool, used before analysis begins.
