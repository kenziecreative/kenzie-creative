...
*Surface what the analysis means. The signal is in what the numbers imply, not what they say.*

Insight is the fourth stage of the Strategy Spine, and it runs on Imagining: seeing the pattern the data is making requires stepping back from the data. For where Insight sits in the spine and why, see the [Strategy Spine overview](../README.md). This page covers what the stage produces, how to choose among its tools, and how it runs.

## What it produces

The small number of load-bearing insights, each in the visual form that makes it undeniable. Recorded in `strategy/brief.md` under Insight.

The stage is done when:

- **Each insight is what the analysis *means*,** not a restatement of what it *said*. A number that moved is a finding; the reason it matters is an insight.
- **The non-obvious truth or tension is named,** not buried inside a chart.
- **The form fits the insight:** the reader sees the pattern without being walked through it.
...
- [Pyramid](pyramid.md) — ideas layered from foundation to apex.
- [Driver Tree](driver-tree.md) — a headline outcome decomposed into the levers that move it, with the rationale on each branch.
...
One strong insight per finding beats a deck of charts. If a form needs a paragraph to explain it, it is carrying the insight for you instead of showing it.

## How it runs

The `strategist-stage` skill runs this stage. Working from the findings Analyse produced, it:

1. Steps back from each finding to name what it *means* — the pattern, the tension, the non-obvious truth.
2. Helps you choose the visual form that makes that insight undeniable, using the menu above.
3. Records the insight and its form in `strategy/brief.md` under Insight.
4. Advances `strategy/STATE.md` to Synthesise once the load-bearing insights are surfaced.

The loop runs backward when it has to. If the insight reveals the analysis was aimed at the wrong dimension, the stage points you back to Frame or Analyse.
