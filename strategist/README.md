...
actual situation, and writes the result into one living strategy brief. A critic can
pressure-test your reasoning whenever you want it.
...
it with you, and writes the result into `strategy/brief.md`. The brief is the artefact —
readable end to end as a complete strategy. `strategy/STATE.md` tracks where you are; the
loop is resumable across sessions.
...
Analyse data pointing one way while Decide commits the other). It reports findings and
records them; acting on them is your call, made back in the relevant stage. If your
thinking holds, it says so — it doesn't manufacture concerns.
