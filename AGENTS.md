...
- **blueprint** (0.2.0) — a process extraction interviewer with a discovery front-door: three jobs across three skills/commands — **discover** (a recall sweep that surfaces recurring work into a thin Process Inventory, no autonomy ratings), **capture** (quick or deep, turning one named process into a structured Process Blueprint with per-step Automate / Monitor / Human ratings and flagged unknowns), and **guide** (orients the user to the right entry point). Non-invention carries across all three. *Standalone.*
