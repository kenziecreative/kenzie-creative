...
- **The goal-setting worktree is NOT disposable yet:** `kenzie-build-goal-setting` (branch `convergence/goal-setting`, fully merged) holds the only copy of the gitignored eval run — `eval/targets/goal-setting/_eval/iteration-1/` (scorecard + all 25 captures). Before deleting the worktree, copy `_eval/` into the primary checkout's matching path. The local-only `dev/convergence/` docs exist in both checkouts.
