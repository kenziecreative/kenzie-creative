...
This is the same operation a scheduled task performs; it just runs it now instead of on the schedule. The skill handles the cadence window, the ledger, suite-mode contribution to the candidate queue (if configured), and writing the dated brief to the briefs directory.
