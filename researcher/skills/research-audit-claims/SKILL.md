...
5. **For every factual claim in the document:**
   ...
   - If `research/cross-reference.md` records a contradiction resolution with `user_override=true` that this claim rests on, does the draft visibly label the override at the finding site AND list it in the Methodology & Limitations section? An overridden resolution presented as if the evidence produced it is **Override undisclosed**.
...
A draft passes when:
- Zero high-severity issues (unsupported claims, misrepresented data, number drift, undisclosed overrides, identity exposure)
...
| Override laundering — a commissioner override reaching the deliverable as if the evidence produced it | When cross-reference.md records `user_override=true` on a resolution this draft relies on, verify the label survived into the draft (finding site + Methodology & Limitations). The internal record is not the disclosure — the reader of the output must see that the commissioner chose against the evidence assessment. |
